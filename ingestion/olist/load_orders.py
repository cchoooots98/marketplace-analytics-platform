"""Load Olist orders CSV data into the raw BigQuery orders table."""
...
RAW_ORDERS_TABLE_ID = "raw_olist.orders"
...
def load_orders_csv(
    csv_path: str | Path,
    *,
    table_id: str = RAW_ORDERS_TABLE_ID,
    write_mode: WriteMode = DEFAULT_WRITE_MODE,
    client: bigquery.Client | None = None,
    project_id: str | None = None,
    location: str | None = None,
) -> BigQueryWriteResult:
    """Load the Olist orders CSV into the raw BigQuery orders table.

    Args:
        csv_path: Local path to the Olist orders CSV file.
...
def prepare_orders_dataframe(
    csv_path: str | Path,
    *,
    metadata: BatchMetadata | None = None,
) -> pd.DataFrame:
    """Prepare the Olist orders CSV for raw BigQuery loading.
    ...
    Returns:
        Orders DataFrame ready to load into the raw BigQuery table.
    """
    return prepare_raw_dataframe(csv_path, ORDERS_SPEC, metadata=metadata)


def main(argv: Sequence[str] | None = None) -> int:
    """Run the Olist orders loader from the command line.
    ...
    Returns:
        Process exit code.
    """
    return run_olist_loader(argv, ORDERS_SPEC)

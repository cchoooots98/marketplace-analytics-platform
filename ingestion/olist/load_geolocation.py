"""Load Olist geolocation CSV data into the raw BigQuery table."""
...
RAW_GEOLOCATION_TABLE_ID = "raw_olist.geolocation"
...
GEOLOCATION_SPEC = OlistRawTableSpec(
    ...
    table_id=RAW_GEOLOCATION_TABLE_ID,
    required_columns=REQUIRED_GEOLOCATION_COLUMNS,
    default_file_name="olist_geolocation_dataset.csv",
)


def load_geolocation_csv(
    csv_path: str | Path,
    *,
    table_id: str = RAW_GEOLOCATION_TABLE_ID,
    write_mode: WriteMode = DEFAULT_WRITE_MODE,
    client: bigquery.Client | None = None,
    project_id: str | None = None,
    location: str | None = None,
) -> BigQueryWriteResult:
    """Load the Olist geolocation CSV into the raw BigQuery table.

    Args:
        csv_path: Local path to the Olist geolocation CSV file.
...
def prepare_geolocation_dataframe(
    csv_path: str | Path,
    *,
    metadata: BatchMetadata | None = None,
) -> pd.DataFrame:
    """Prepare the Olist geolocation CSV for raw BigQuery loading.
    ...
    Returns:
        Geolocation DataFrame ready to load into the raw BigQuery table.
    """
    return prepare_raw_dataframe(csv_path, GEOLOCATION_SPEC, metadata=metadata)


def main(argv: Sequence[str] | None = None) -> int:
    """Run the Olist geolocation loader from the command line.
    ...
    Returns:
        Process exit code.
    """
    return run_olist_loader(argv, GEOLOCATION_SPEC)

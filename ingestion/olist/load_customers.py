"""Load Olist customers CSV data into the raw BigQuery table."""
...
RAW_CUSTOMERS_TABLE_ID = "raw_olist.customers"
...
CUSTOMERS_SPEC = OlistRawTableSpec(
    ...
    table_id=RAW_CUSTOMERS_TABLE_ID,
...
def load_customers_csv(
    csv_path: str | Path,
    *,
    table_id: str = RAW_CUSTOMERS_TABLE_ID,
    write_mode: WriteMode = DEFAULT_WRITE_MODE,
    client: bigquery.Client | None = None,
    project_id: str | None = None,
    location: str | None = None,
) -> BigQueryWriteResult:
    """Load the Olist customers CSV into the raw BigQuery table.

    Args:
        csv_path: Local path to the Olist customers CSV file.
...
def prepare_customers_dataframe(
    ...
    *,
    ...
    """Prepare the Olist customers CSV for raw BigQuery loading.
    ...
    Returns:
        Customers DataFrame ready to load into the raw BigQuery table.
    """
    return prepare_raw_dataframe(csv_path, CUSTOMERS_SPEC, metadata=metadata)


def main(argv: Sequence[str] | None = None) -> int:
    """Run the Olist customers loader from the command line.
    ...
    Returns:
        Process exit code.
    """
    return run_olist_loader(argv, CUSTOMERS_SPEC)

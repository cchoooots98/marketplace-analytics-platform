...
class FakeLoadJob:
    ...
    def __init__(self, job_id: str = "job_123", output_rows: int | None = 2) -> None:
        self.job_id = job_id
...
class FakeBigQueryClient:
    ...
    def __init__(self, load_job: FakeLoadJob) -> None:
...
def test_write_dataframe_to_bigquery_replaces_table_contents() -> None:
    """Validate replace-mode DataFrame loads.
    ...
    load_job = FakeLoadJob(job_id="replace_job", output_rows=1)
    fake_client = FakeBigQueryClient(load_job)
    ...
    assert fake_client.destination_table == "project.raw_ext.holidays"
    assert fake_client.job_config is not None
    assert (
        fake_client.job_config.write_disposition
